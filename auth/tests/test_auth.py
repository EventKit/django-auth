...
class TestAuth(TestCase):
    ...
    def test_get_user(self):
        ...
        with patch("auth.auth.authenticate") as mock_authenticate:
    ...
    def test_request_access_token(self):
        ...
        self.mock_requests.post(
            settings.OAUTH_TOKEN_URL,
            text=json.dumps({settings.OAUTH_TOKEN_KEY: example_access_token}),
            status_code=200,
        )
        ...
        with patch("auth.auth.requests.post") as mock_post:
    ...
    @patch("auth.auth.get_user_data_from_schema")
    ...
    def test_fetch_user_from_token(self, mock_get_user_data, mock_get_user):
        ...
        self.mock_requests.get(
            settings.OAUTH_PROFILE_URL, text=json.dumps(user_data), status_code=200
        )
        ...
        with patch("auth.auth.requests.get") as mock_post:
            mock_post.side_effect = requests.ConnectionError()
            with self.assertRaises(OAuthServerUnreachable):
