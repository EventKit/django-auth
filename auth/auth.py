...
from auth.helpers import set_session_user_last_active_at
...
def auto_logout(get_response):
    def middleware(request):
        ...
        if getattr(settings, "AUTO_LOGOUT_SECONDS"):
            last_active_at_iso = request.session.get(
                settings.SESSION_USER_LAST_ACTIVE_AT
            )
            if last_active_at_iso:
                ...
                time_passed = (
                    datetime.utcnow().replace(tzinfo=pytz.utc) - last_active_at
                )
